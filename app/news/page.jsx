...
const portalsApiUrl = `${serverUrl.replace(/\/+$/, '')}/api/portals`
...
async function getNews({ page, limit, date, source_name, title } = {}) {
  try {
    ...
    if (source_name) url.searchParams.set('source_name', String(source_name))
...
async function getPortals() {
  try {
    const response = await fetch(portalsApiUrl, { cache: 'no-store' })

    if (!response.ok) {
      throw new Error(`Falha ao carregar portais: ${response.status}`)
    }

    const data = await response.json()

    return Array.isArray(data) ? data.filter((portal) => typeof portal === 'string' && portal.trim()) : []
  } catch (error) {
    return []
  }
}
...
export default async function NewsPage({ searchParams }) {
  ...
  const sourceParam = getParam(searchParams?.source_name)
  const titleParam = getParam(searchParams?.title)

  const [newsResult, portals] = await Promise.all([
    getNews({
    page: pageParam,
    limit: limitParam || undefined,
    date: dateParam || undefined,
    source_name: sourceParam || undefined,
    title: titleParam || undefined,
    }),
    getPortals(),
  ])

  const { news, pagination, error } = newsResult

  const buildNewsHref = (
    targetPage,
    nextDate = dateParam,
    nextSource = sourceParam,
    nextTitle = titleParam,
  ) => {
    ...
    if (nextSource) {
      params.set('source_name', nextSource)
    }
